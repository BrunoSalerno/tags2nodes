exports.Osm=function(osmread, FILE_PATH, Q,_){
  ...
  function print_data() {
    ...
    // " ways: " + ways + " relations: " + relations+"\r");
  ...
  this.parse = function(tag_conditions,element,callback){
    ...
    osmread.parse({
      ...
      node: function(node){
        ...
        if (element='node'){
          for (condition in tag_conditions) {
              ...
              } else if (node.tags[condition] &&
                node.tags[condition] == tag_conditions[condition]) {
      ...
      way: function(way){
        ...
        if (element='way'){
          for (condition in tag_conditions) {
              if (way.tags[condition] &&
                way.tags[condition] == tag_conditions[condition]) {
                output.push(way)
              }
          }
        }
      ...
      relation: function(relation){
        ...
        if (element='relation'){
          for (condition in tag_conditions) {
              if (relation.tags[condition] &&
                relation.tags[condition] == tag_conditions[condition]) {
  ...
  this.fetch_and_save_nodes = function(table,db, nodes, tags_type,
                                       extra_condition,callback) {
