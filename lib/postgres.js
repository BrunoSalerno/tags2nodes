exports.Query = function(){
	...
	this.create_table = function(table, tags_type, callback){		
		self.client.query('DROP TABLE IF EXISTS '+table, function(err,result){
			...
			self.client.query('CREATE TABLE '+table+'(id bigint primary key, tags ' + tags_type + ')', function(err,result){
				if(err) return console.error('Error running query', err);
							
				self.client.query("SELECT AddGeometryColumn('"+table+"', 'geo',"+self.proj+" , 'POINT', 2 );", function(err,result){
						if(err) return console.error('Error running query', err);
	...
	this.add_row = function(table, id, tags, lat, lon, callback){		
		...
		self.client.query("INSERT INTO "+table+" VALUES ("+id+",'"+JSON.stringify(tags)+"',"+geo+")", function(err,result){
			if(err) return console.error('Error running query', err);
			if (typeof callback === 'function') callback();
		});
	}
